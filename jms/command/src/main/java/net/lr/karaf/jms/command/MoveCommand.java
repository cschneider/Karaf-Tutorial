...
@Command(scope = "jms", name = "move", description = "move one or more messages from a queue to another destination")
public class MoveCommand implements Action {
    ...
    @Argument(index = 1, name = "destName", required = true, description = "destination to send to", multiValued = false)
    String destName;
    
    @Argument(index = 2, name = "messageId", required = false, description = "Message ID", multiValued = false)
    String messageId;
    
    @Option(name = "-s", description = "JMS Selector for the messages to move")
    String selector;
    
    @Option(name = "-a", description = "Move all messages")
    boolean all;
